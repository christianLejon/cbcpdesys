...
if not folder is None:
    ...
    folder = path.join(getcwd(), 'channel395', 
...
no_slip = DirichletBC(V, Constant(0), walls)
one_on_wall = DirichletBC(V, Constant(1), walls)
periodic1 = PeriodicBC(V, pbx)
periodic2 = PeriodicBC(V, pbz)
bc = [periodic1, periodic2, no_slip]
bcT = [periodic1, periodic2, one_on_wall]
bcs['u0'] = bc
bcs['u1'] = bc
bcs['u2'] = bc
bcs['T'] = bcT
...
x_1 = dict((ui, q_1[ui].vector()) for ui in u_components + ['T']) # Solution vectors t - dt
...
while t < T + DOLFIN_EPS:
    ...
    while err > max_error and j < num_iter:
        ...
        if j == 1:
            ...
            for i, ui in enumerate(u_components+['T']):
                b[ui][:] = b0[ui][:]
    ...
    # Solving scalar
    [bc.apply(b['T']) for bc in bcs['T']]
    if MPI.process_number() == 0:
        print 'Solving scalar '
    t0 = time.time()
    u_sol.solve(A, x_['T'], b['T'])
    u_sol.t += (time.time()-t0)
    bcs['u0'][0].post_solve(V._periodic_master_slave_dofs, x_['T'])
