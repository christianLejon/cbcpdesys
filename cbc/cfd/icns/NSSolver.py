...
def __missing__(self, key):
    ...
    We will here overload such that the prm dictionary looks for keys as:
       from cbc.cfd.icns.NSProblem import *
       1) prm[key]['u1']
       2) prm[key]['u']
       3  prm[key]
...
class NSSolver(PDESystem):
    ...
    def setup_subsystems(self):
        ...
        #pdb.set_trace()
        # See setup(self): in PDESystem. There is a need for adaptivity because Tensor/Vector/FunctionSpace have different call synthax. By first declaring an empty dict self.prm['symmetry']={} the code will be synthactically correct for the channel.py demo, but perhaps not for arbitrary choice of Tensor/Vector/FunctionSpace ???
        self.prm['symmetry']={} # added line
