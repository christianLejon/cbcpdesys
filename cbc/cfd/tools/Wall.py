...
class Wallfunction:
        ...
        for i in vertices_on_boundary:
            ...
            if i in v2c:
                for ci in v2c[i]:
                    ...
                    x = mesh.coordinates()[a] # This may be a workaround,,, ? x gives the coordinates of the vertices "a" of cell "c" where "c" has vertice "i" on the boundary
                    aa = list(a) # aa is an list of the vertices of the cell that has at least one vertice on the boundadry
                    ii = aa.index(i) # The local index of the boundary node
                    yy = x[ii]       # Coordinates of the boundary node
                    for kk, jj in enumerate(aa):  # Loop through vertices of the cell
    ...
    def get_vertices_nearest(self, bnd_to_in):
        vertices_nearest = []
        #for v in bnd_to_in.values(): # This works as well I think /CL
        for v in iter(bnd_to_in.values()):
    ...
    def handle_corners(self, V, vertices_on_boundary,
        ...
        for i in vertices_on_boundary:
            if i in v2c:
                if len(v2c[i]) == 1:
                    ci = v2c[i][0]
                    # c = Cell(mesh,ci) # c är ett cell-objekt för en cell på randen
                    a = dofmap.cell_dofs(ci)
    ...
    def map_cells_and_vertices(self, V):
        ...
        for c in cells(mesh):
            c2v[c.index()] = []
            a = dofmap.cell_dofs(c.index()) # Here replaced tabulate_dofs to cell_dofs and two more places
            for v in a:
    ...
    def get_vertices_on_boundary(self, V, bc):
        ...
        u.vector()[:] = 0
        d.apply(u.vector())
        return set(array(find(abs(u.vector().get_local() - 1.) < DOLFIN_EPS),
                         dtype='int32'))
    ...
    def apply(self, *args):
        ...
        for var in args:
            if isinstance(var, (Matrix, PETScMatrix)):
                ...
                PETSc_MAT_NEW_NONZERO_ALLOCATION_ERR = 19  # HG
                PETSc_FALSE = 0  # HG
                var.mat().setOption(PETSc_MAT_NEW_NONZERO_ALLOCATION_ERR, PETSc_FALSE)  # HG
